...
python3 -m pytest tests/ -m "not integration and not slow"   # быстрые: 203 passed, 42 deselected (герметичны, postgres НЕ нужен)
...
├── tests/                   # tests/core/, tests/storage/, tests/api/, tests/fakes.py, tests/test_*.py — 203 fast + 41 integration + 1 slow
...
- **Счётчики** — 203 fast (42 deselected), 41 integration, 1 slow
