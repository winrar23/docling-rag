...
- Документация разбросана по PDF, DOCX и Confluence — сложно искать
...
[1] score=0.720 | architecture.md | стр.1 | text
    DWH использует схему звезда с таблицами фактов и измерений...

[2] score=0.651 | design.pdf | стр.4 | text
...
chunk_size: 3200                     # размер chunk в символах (≈800 токенов)
chunk_overlap: 320                   # overlap между chunks (10%)
...
> **Важно:** нельзя менять `embedding_model` после того, как документы уже проиндексированы — размерность векторов изменится. Для смены модели нужно переиндексировать всё заново.
...
| P2 | Vision LLM для изображений/диаграмм | 💡 |
| P2 | Веб-интерфейс | 💡 |
